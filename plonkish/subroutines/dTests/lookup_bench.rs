...
use std::{error::Error, iter::zip, mem::take, sync::Arc, time::Instant};
use subroutines::{
    instruction::xor::XORInstruction, pcs::prelude::MultilinearKzgPCS, JoltInstruction,
    LookupCheck, MultilinearProverParam, MultilinearVerifierParam, PolyIOP,
    PolynomialCommitmentScheme,
};

fn read_mkzg_srs() -> Result<
    (
        MultilinearProverParam<Bn254>,
        MultilinearVerifierParam<Bn254>,
    ),
    Box<dyn Error>,
> {
    let sub_prover_setup_filepath = format!("mkzg-max{}.paras", MAX_NUM_VARS);
    let verifier_setup_filepath = format!("mkzgVerifier-max{}.paras", MAX_NUM_VARS);
    let prover_setup = {
...
>(
    ...
    for _ in 0..repetition {
        let _ = <PolyIOP<E::ScalarField> as LookupCheck<
            E,
            MultilinearKzgPCS<E>,
            Instruction,
            C,
    ...
    let mut poly = <PolyIOP<E::ScalarField> as LookupCheck<
        ...
        M,
    >>::construct_polys(&preprocessing, ops, &alpha);
    let proof_ret = <PolyIOP<E::ScalarField> as LookupCheck<
        E,
        MultilinearKzgPCS<E>,
        Instruction,
        C,
    ...
    let mkzg_pcs_srs = {
        match read_mkzg_srs() {
            ...
            Err(_e) => {
                ...
                let srs =
                    MultilinearKzgPCS::<Bn254>::gen_srs_for_testing(&mut srs_rng, 24).unwrap();
