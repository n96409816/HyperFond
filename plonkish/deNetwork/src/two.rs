...
/// Are you the first party in the De?
#[inline]
...
impl DeNet for DeTwoNet {
    ...
    fn recv_bytes_from_master(bytes: Option<Vec<Vec<u8>>>) -> Vec<u8> {
        let mut ch = get_ch!();
        ch.stats.from_master += 1;
        if ch.talk_first {
            let mut bytes = bytes.expect("master needs bytes");
    ...
    fn recv_bytes_from_master_uniform(bytes: Option<Vec<u8>>) -> Vec<u8> {
        ...
        ch.stats.from_master += 1;
        if ch.talk_first {
            let bytes = bytes.expect("master needs bytes");
            ch.send_slice(&bytes);
            bytes
        } else {
            ch.recv_vec()
