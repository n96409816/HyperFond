...
pub trait DeSerNet: DeNet {
    #[inline]
    fn broadcast<T: Serialize + DeserializeOwned>(out: &T) -> Vec<T> {
    ...
    #[inline]
    fn send_to_master<T: Serialize + DeserializeOwned>(out: &T) -> Option<Vec<T>> {
        // let mut bytes_out = Vec::new();
        // out.serialize_uncompressed(&mut bytes_out).unwrap();
        let bytes_out = bincode::serialize(out).unwrap();
        println!("send bytes: {}", bytes_out.len(),);
    ...
    #[inline]
    fn recv_from_master<T: Serialize + DeserializeOwned + Default>(out: Option<Vec<T>>) -> T {
        if Self::am_master() {
            let bytes = out
                .as_ref()
                .unwrap()
    ...
    #[inline]
    fn recv_from_master_uniform<T: Serialize + DeserializeOwned + Default>(out: Option<T>) -> T {
        if Self::am_master() {
            // let mut bytes_out = Vec::new();
            // out.as_ref()
            //     .unwrap()
            ...
            //     .unwrap();
            let bytes_out = bincode::serialize(out.as_ref().unwrap()).unwrap();
    ...
    #[inline]
    fn atomic_broadcast<T: Serialize + DeserializeOwned>(out: &T) -> Vec<T> {
        // let mut bytes_out = Vec::new();
        // out.serialize_compressed(&mut bytes_out).unwrap();
        let mut bytes_out = bincode::serialize(out).unwrap();
    ...
    #[inline]
    fn king_compute<T: Serialize + DeserializeOwned + Default>(
        x: &T,
...
#[inline]
pub fn exchange<F: Serialize + DeserializeOwned>(f: &F) -> F {
    // let mut bytes_out = Vec::new();
    // f.serialize_compressed(&mut bytes_out).unwrap();
    let bytes_out = bincode::serialize(f).unwrap();
...
pub fn atomic_exchange<F: Serialize + DeserializeOwned>(f: &F) -> F {
    // let mut bytes_out = Vec::new();
    // f.serialize_compressed(&mut bytes_out).unwrap();
    let mut bytes_out = bincode::serialize(f).unwrap();
    let ser_len = bytes_out.len();
    bytes_out.resize(ser_len + COMMIT_RAND_BYTES, 0);
    rand::thread_rng().fill_bytes(&mut bytes_out[ser_len..]);
    let commitment = CommitHash::new().chain(&bytes_out).finalize();
    ...
    bincode::deserialize(&other_bytes[..ser_len]).unwrap()
}
