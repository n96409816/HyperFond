...
use plonkish_backend::{
    backend::{
        self,
        hyperplonk::{
            util::{rand_vanilla_plonk_assignment, vanilla_plonk_expression},
    ...
    util::{
        ...
        transcript::{Blake2sTranscript, FiatShamirTranscript, InMemoryTranscript},
    },
};
...
fn main() {
    ...
    // system.bench(18, circuit);
    // system.bench(22, circuit);
    // system.bench(23, circuit);
    // system.bench(25, circuit);
    // system.bench(24, circuit);
    // system.bench(23, circuit);
    system.bench(22, circuit);
    // system.bench(28, circuit);
}
...
fn bench_hyperplonk_18_8<C: CircuitExt<GoldilocksMont>>(k: usize) {
    ...
    HyperPlonk::preprocess_p(&param, &circuit_info).unwrap();
    HyperPlonk::preprocess_v(&param, &circuit_info).unwrap();
    end_timer(timer);

    let sub_prover_setup_filepath = format!("../data/SubProver-{}.paras", k);
    ...
    println!("proving for {} variables: {:?}", k, start.elapsed(),);
    let bytes_out = bincode::serialize(&proof).unwrap();
    println!("proof bytes: {}", bytes_out.len(),);

    drop(pp);
    let verifier_setup_filepath = format!("../data/Verifier-{}.paras", k);
    ...
    println!("verifiy for {} variables: {:?}", k, start.elapsed(),);
    assert!(accept);
}
...
//     let vp = HyperPlonkVerifierParam::read_from_file(&verifier_setup_filepath).unwrap();
//     let bytes_out = bincode::serialize(&proof).unwrap();
//     println!("proof size {}", bytes_out.len());
//     let _timer = start_timer(|| format!("hyperplonk_verify-{k}"));
//     let mut transcript = Blake2sTranscript::from_proof((), proof.as_slice());
//     let start = Instant::now();
//     let accept = HyperPlonk::verify(&vp, instances, &mut transcript, std_rng()).is_ok();
...
impl System {
    ...
    fn bench(&self, k: usize, circuit: Circuit) {
        ...
        match k {
            ...
            // 28 => bench_hyperplonk_28_8::<VanillaPlonk<GoldilocksMont>>(k),
            _ => {}
        }
    }

    // fn bench(&self, m: usize, circuit: Circuit) {
    //     use ark_std::collections::HashMap;
    //     use ark_std::ptr::addr_of;
    //     use plonkish_backend::util::expression::Expression;
    //     use std::ops::Deref;
    //     type Basefold = multilinear::Basefold<GoldilocksMont, Blake2s256, Eighteen8>;
    //     type HyperPlonk = backend::hyperplonk::HyperPlonk<Basefold>;

    //     let circuit = VanillaPlonk::rand(m, std_rng());
    //     let circuit = Halo2Circuit::new::<HyperPlonk>(m, circuit);
    //     let circuit_info = circuit.circuit_info().unwrap();
    //     let instances = circuit.instances();

    //     let timer = start_timer(|| format!("hyperplonk_setup-{m}"));
    //     let param = HyperPlonk::setup(&circuit_info, std_rng()).unwrap();
    //     end_timer(timer);
    //     use plonkish_backend::util::transcript::FieldTranscript;

    //     let timer = start_timer(|| format!("hyperplonk_preprocess-{m}"));
    //     HyperPlonk::preprocess_p(&param, &circuit_info).unwrap();
    //     HyperPlonk::preprocess_v(&param, &circuit_info).unwrap();
    //     end_timer(timer);

    //     let sub_prover_setup_filepath = format!("../data/SubProver-{}.paras", m);
    //     let pp = HyperPlonkProverParam::<GoldilocksMont, Basefold>::read_from_file(
    //         &sub_prover_setup_filepath,
    //     )
    //     .unwrap();
    ...
    //     // transcript.common_field_element(&GoldilocksMont::ONE);
    //     let t = transcript.squeeze_challenges(m);

    //     let eq_xt = MultilinearPolynomial::<GoldilocksMont>::eq_xy(&t);

    //     let evals: Vec<_> = pp.preprocess_polys[0]
    //         .evals
    //         .iter()
    //         .zip(eq_xt.evals[..pp.preprocess_polys[0].evals.len()].iter())
    //         .map(|(x, y)| *x * *y)
    //         .collect();
    //     // let evals = pp.preprocess_polys[0].evals;
    //     let poly = MultilinearPolynomial::<GoldilocksMont>::new(evals.clone());

    //     let expression = Expression::<GoldilocksMont>::Polynomial(Query::new(0, Rotation::cur()))
    //         * Expression::<GoldilocksMont>::eq_xy(0);
    //     let mut polys = Vec::with_capacity(1);
    //     polys.push(poly);
    //     let binding = [t];
    //     let virtual_poly = VirtualPolynomial::new(&expression, polys.iter(), &[], &binding);

    //     let tilde_gs_sum = inner_product(evals.iter(), &eq_xt[..evals.len()]);

    //     let (_challenges, _) = ClassicSumCheck::<EvaluationsProver<_>>::prove(
    //         &(),
    //         m,
    //         virtual_poly,
    //         // tilde_gs_sum,
    ...
    //     let mut transcript = Blake2sTranscript::from_proof((), proof.as_slice());
    //     let t: Vec<GoldilocksMont> = transcript.squeeze_challenges(m);

    //     let timer = start_timer(|| format!("sumcheck verify-{m}"));
    //     let (_g_prime_eval, _verify_point) = ClassicSumCheck::<EvaluationsProver<_>>::verify(
    //         &(),
    //         m,
    //         expression.degree(),
    //         // tilde_gs_sum,
