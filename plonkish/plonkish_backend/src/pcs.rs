...
pub trait PolynomialCommitmentScheme<F: Field>: Clone + Debug {
    ...
    fn d_batch_commit<'a>(
        pp: &Self::ProverParam,
        polys: impl IntoIterator<Item = &'a Self::Polynomial>,
    ) -> Result<Vec<Option<Vec<Self::Commitment>>>, Error>
    where
        Self::Polynomial: 'a,
    {
        ...
        let coms = polys_vec
            .iter()
            .map(|poly| Self::d_commit(pp, poly))
    ...
    where
        Self::Polynomial: 'a,
    {
        let comms = Self::d_batch_commit(pp, polys)?;

        if Net::am_master() {
            for comm in comms.iter() {
                for item in comm.clone().unwrap().iter() {
                    transcript.write_commitments(item.as_ref())?;
                }
            }
        }
        Ok(comms)
