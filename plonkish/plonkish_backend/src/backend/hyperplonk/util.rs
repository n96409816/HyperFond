...
pub(super) fn computer_nums_and_denoms<F: PrimeField>(
...
) -> Result<(MultilinearPolynomial<F>, MultilinearPolynomial<F>), PolyIOPErrors> {
    let start = start_timer!(|| "compute numerators and denominators");

    let num_vars = fxs.num_vars;
    let s_ids = identity_permutation_mles::<F>(num_vars, 1);
    // for l in 0..fxs.len() {
    let mut numerator_evals = vec![];
    let mut denominator_evals = vec![];

    for (&f_ev, (&g_ev, (&s_id_ev, &perm_ev))) in fxs
        .iter()
        .zip(gxs.iter().zip(s_ids[0].iter().zip(perms.iter())))
    {
    ...
    end_timer!(start);
    Ok((
        MultilinearPolynomial::from_evals(numerator_evals),
        MultilinearPolynomial::from_evals(denominator_evals),
    ))
}
...
pub fn identity_permutation_mles<F: PrimeField>(
    ...
    for i in 0..num_chunks {
        ...
        res.push(MultilinearPolynomial::from_evals(s_id_vec));
    }
    res
}
