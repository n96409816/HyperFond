...
pub(super) fn compose<F: PrimeField>(circuit_info: &PlonkishCircuitInfo<F>) -> (Expression<F>) {
    ...
    let [alpha] = &array::from_fn(|idx| Expression::<F>::Challenge(challenge_offset + idx));
