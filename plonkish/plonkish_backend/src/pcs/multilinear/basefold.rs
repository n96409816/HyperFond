    ...
    V: BasefoldExtParams,
{
    ...
    fn d_commit(
        sub_prover_param: &Self::ProverParam,
        poly: &Self::Polynomial,
    ) -> Result<Option<Vec<Self::Commitment>>, Error> {
        ...
        let mut sub_commitment = Type1Polynomial::default();
        if (sub_prover_param.rs_basecode) {
            let mut basecode = encode_rs_basecode(
                &coeffs,
                1 << sub_prover_param.log_rate,
                1 << (sub_prover_param.num_vars - sub_prover_param.num_rounds),
            );
            assert_eq!(basecode.poly.len() > 0, true);

            sub_commitment = d_evaluate_over_foldable_domain_2(
                m,
                sub_prover_param.num_vars - sub_prover_param.num_rounds + sub_prover_param.log_rate,
                sub_prover_param.log_rate,
                basecode,
                &sub_prover_param.table,
            );
        } else {
            sub_commitment = d_evaluate_over_foldable_domain(
                m,
                sub_prover_param.log_rate,
                coeffs,
                &sub_prover_param.table,
                sub_prover_param.code_type.clone(),
            );
        }

        let sub_tree = merkelize::<F, H>(&sub_commitment);

        let sub_comm = Self::Commitment {
            codeword: sub_commitment,
            codeword_tree: sub_tree,
            bh_evals: bh_evals,
        };
...
pub fn d_evaluate_over_foldable_domain<F: PrimeField>(
    log_m: usize,
    log_rate: usize,
    mut coeffs: Type2Polynomial<F>,
    table: &Vec<Vec<F>>,
...
pub fn d_evaluate_over_foldable_domain_2<F: PrimeField>(
    log_m: usize,
    log_chunk_size: usize,
    log_rate: usize,
    mut coeffs: Type2Polynomial<F>,
    ...
    //iterate over array, replacing even indices with (evals[i] - evals[(i+1)])
    let mut chunk_size = 1 << log_chunk_size;
    let levels = log2_strict(coeffs.poly.len()) - log_chunk_size - log_m;
