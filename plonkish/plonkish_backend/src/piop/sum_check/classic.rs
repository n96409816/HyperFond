    ...
    P: ClassicSumCheckProver<F>,
{
    ...
    fn d_prove(
        ...
        let sub_evals = state.into_evals();

        let r_evals = Net::send_to_master(&sub_evals[0]);
        if Net::am_master() {
            let timer = start_timer(|| format!("distributd sumcheck master"));
            let evals = r_evals.unwrap();
            assert_eq!(evals.len(), 1 << m);
            let sum = evals.iter().fold(F::ZERO, |acc, item| acc + item);
            let poly = MultilinearPolynomial::<F>::new(evals.clone());

            let _ = transcript.write_field_element(&sum);

            let expression = Expression::<F>::Polynomial(Query::new(0, Rotation::cur()));
            let mut polys = Vec::with_capacity(1);
            polys.push(poly);
            let virtual_poly = VirtualPolynomial::new(&expression, polys.iter(), &[], &[]);
    ...
    fn d_verify(
...

        {
            ...
            let sum = transcript.read_field_element()?;
            for _ in 0..m {
                msgs.push(P::RoundMessage::read(1, transcript)?);
                challenges.push(transcript.squeeze_challenge());
            }
            P::RoundMessage::verify_consistency(1, sum, &msgs, &challenges)?;
        }
